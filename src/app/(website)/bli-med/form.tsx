...
const Form = () => {
  ...
  const [fallbackMessage, setFallbackMessage] = useState<React.JSX.Element | null>(null);
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (Object.keys(validationErrors).length === 0) {
      const selectedRoles = Object.keys(formData.role)
        .filter((role) => formData.role[role as keyof typeof formData.role])
      ...
      setFallbackMessage(
        <div className="mt-4 rounded border border-gray-300 bg-gray-100 p-4">
          <p>
            Hvis e-posten klienten din ikke åpner seg automatisk, vennligst kopier følgende
            informasjon og send det til {toEmail}. Takk!
          </p>
  ...
  return (
    <form className="mt-16 flex flex-col space-y-4" onSubmit={handleSubmit}>
      <div>
      ...
      <fieldset className="space-y-2">
        <legend className="block text-sm font-medium text-gray-700">Rolle (velg minst én)</legend>
        {Object.keys(formData.role).map((role) => (
          <label key={role} className="flex items-center text-base">
            <input
              type="checkbox"
              name={role}
              checked={formData.role[role as keyof FormData['role']]}
              onChange={handleChange}
              className="mr-2 h-4 w-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500"
            />
            {roleDisplayNames[role as keyof FormData['role']]}
          </label>
        ))}
      </fieldset>
      {errors.role && (
      ...
      <button
        ...
        className={`mt-4 w-full rounded-md py-2 font-bold shadow ${isSubmitDisabled ? 'bg-gray-300' : 'bg-indigo-600 text-white hover:bg-indigo-700'} focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 focus:outline-none`}
      >
        Send
      </button>
    </form>
