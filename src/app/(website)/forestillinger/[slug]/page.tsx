...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    keywords: ['Lokaltheatret', 'Teater', 'Oslo', 'Forestilling', play.playTitle, play.location],
    openGraph: {
      type: 'website',
      url: url,
      title: title,
...
const Page = async ({ params }: Props) => {
  ...
  return (
    <div className="flex flex-col">
      ...
      />
      <div
        className="grid grid-cols-1 justify-between gap-4 px-[32px] py-8 md:grid-cols-3 md:px-[10vw]"
        style={{ backgroundColor: play.playColor }}
      >
        <div className="flex flex-col items-center" style={{ color: play.textColor }}>
          <p className="font-bold">Forestillingsperiode:</p>
          {play.playDates && play.active && play.playDates.length > 0 ? (
      ...
      <div>
        <div className="page-container">
          {play.active && play.ticketInfo && (
            <TicketButton
              ...
              className="my-4 hidden w-full p-4 md:block"
            />
          )}
        ...
        {play.active && play.ticketInfo && (
          <TicketButton
            ...
            backgroundColor={play.textColor}
            color={play.playColor}
            className="sticky bottom-0 w-full p-4 md:hidden"
          />
        )}
      </div>
    </div>
